...
alter table public.profiles
  ...
  check (role in ('admin', 'president', 'vice_president', 'financial_officer', 'team_lead'));

update public.profiles
set is_vice_president = coalesce(is_vice_president, false) or role = 'vice_president';
...
as $$
begin
  insert into public.profiles (id, full_name, email, role, is_admin, is_president, is_vice_president, is_financial_officer, active)
  values (
    new.id,
    ...
    coalesce(new.raw_user_meta_data ->> 'role', 'team_lead') = 'vice_president',
  ...
  set
    ...
    is_vice_president = profiles.is_vice_president or excluded.is_vice_president,
